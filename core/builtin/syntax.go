...
func (se *syntaxEnv) quote(v ale.Value) (ale.Value, error) {
	return se.quoteValue(v)
}

func (se *syntaxEnv) quoteValue(v ale.Value) (ale.Value, error) {
	switch v := v.(type) {
	case data.Sequence:
...
func (se *syntaxEnv) quoteSymbol(s data.Symbol) ale.Value {
...
func (se *syntaxEnv) quoteSequence(s data.Sequence) (ale.Value, error) {
	switch s := s.(type) {
	case data.String:
		return s, nil
	case *data.List:
...
func (se *syntaxEnv) quoteList(s *data.List) (ale.Value, error) {
	if s == data.Null {
		return s, nil
	}
...
func (se *syntaxEnv) quoteVector(s data.Vector) (ale.Value, error) {
	e, err := se.quoteElements(s)
	if err != nil {
		return nil, err
	}
...
func (se *syntaxEnv) quotePair(c data.Pair) (ale.Value, error) {
...
func (se *syntaxEnv) quoteObject(as *data.Object) (ale.Value, error) {
	var res data.Vector
...
func (se *syntaxEnv) qualifySymbol(s data.Symbol) ale.Value {
...
func isWrapperCall(s data.Symbol, v ale.Value) (ale.Value, bool) {
...
func isBuiltInCall(s data.Symbol, v ale.Value) (*data.List, bool) {
...
func isUnquote(v ale.Value) (ale.Value, bool) {
	return isWrapperCall(unquoteSym, v)
}

func isUnquoteSplicing(v ale.Value) (ale.Value, bool) {
